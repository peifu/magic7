...
def print_mail(mail_data):
    items = ['subject', 'charsets', 'date', 'from', 'to', 'cc', 'content_text', 'content_html', 'attachments']
    for item in items:
        if (item in mail_data):
            rprint(item)
            if (item == 'content_text'):
                print(mail_data[item])
                print(mail_data[item][0:20])
            else:
                print(mail_data[item])
